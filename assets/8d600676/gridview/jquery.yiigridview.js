...
(function($)
{
  ...
  methods = {
    ...
    init: function(options)
    {
      ...
      return this.each(function()
      {
        var eventType,
          ...
          inputSelector = '#' + id + ' .' + settings.filterClass + ' input, ' + '#' + id + ' .' + settings.filterClass +
            ' select';
          ...
          $(document).on('click.yiiGridView', settings.updateSelector, function()
          {
            ...
            if(settings.enableHistory && window.History.enabled)
            {
              ...
              window.History.pushState(null, document.title,
                $.param.querystring(url.substr(0, url.indexOf('?')), params));
            }
            else
            {
        ...
        if(settings.selectableRows > 0)
        {
          ...
          $(document).on('click.yiiGridView', '#' + id + ' .' + settings.tableClass + ' > tbody > tr', function(e)
          {
            ...
            if($target.closest('td').is('.empty,.button-column') ||
            ...
            $("input.select-on-check-all", $currentGrid).prop('checked',
              $checks.length === $checks.filter(':checked').length);
    ...
    getChecked: function(column_id)
    {
      ...
      this.find('.' + settings.tableClass).children('tbody').children('tr').children('td').children('input[name="' +
        column_id + '"]').each(function(i)
      {
        if(this.checked)
        {
