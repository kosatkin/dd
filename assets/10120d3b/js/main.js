$(document).ready(function()
{
  ...
  {
    $('table.preview input[name="checkAll"]').prop('checked',
      !$('table.preview td.confirm input:not(:checked)').length);
  });
    ...
    $.ajax({
      ...
      success: function(data)
      {
        $.fancybox(data, {
          ...
          {
            return '<div id="tip7-title"><span><a href="javascript:;" onclick="$.fancybox.close();">close</a></span>' +
              (title && title.length?'<b>' + title + '</b>':'' ) + '</div>';
          },
